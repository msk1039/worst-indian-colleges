...
export default function CollegeList() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "UPDATE", schema: "public", table: "colleges-t2" },
        (payload) => {
          setColleges((prevColleges) =>
            prevColleges.map((college) =>
              college.id === payload.new.id
                ? { ...college, ...payload.new }
                : college
            )
          );
  ...
  useEffect(() => {
    ...
    const sortedFilteredColleges =
      sortOrder === "desc" ? filteredColleges : [...filteredColleges].reverse();
    const indexOfLastCollege = currentPage * collegesPerPage;
    const indexOfFirstCollege = indexOfLastCollege - collegesPerPage;
    setCurrentColleges(
      sortedFilteredColleges.slice(indexOfFirstCollege, indexOfLastCollege)
    );
  ...
  if (loading) {
    return (
      <div className="w-full h-1  bg-gradient-to-br from-purple-700 via-purple-500 to-yellow-400"></div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-background text-foreground p-8 font-sans">
      <div className="max-w-6xl mx-auto">
        ...
        <h3 className="text-xl font-medium mb-4 text-left animate-text bg-gradient-to-r from-teal-500 via-purple-500 to-orange-500 bg-clip-text text-transparent text-5xl font-black">
          Vote for which is the WORST indian college out there. Click the search
          bar, search your college and HIT the VOTE BUTTON
        </h3>
            ...
            <TableBody>
              {currentColleges.map((college) => (
                <Link
                  ...
                  passHref
                >
                  <TableRow className="transition ease-out duration-300 hover:ease-in hover:duration-300 hover:bg-muted/50  hover:shadow-md cursor-pointer group ">
                    <TableCell className="font-medium">
                      {college.rank}
                    </TableCell>
                    <TableCell>
                      <span className="group-hover:text-primary group-hover:underline transition-colors duration-200">
                        {college.name}
                      </span>
                    </TableCell>
